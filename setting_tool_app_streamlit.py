...
with st.expander("ヒドラ目関連"):
    ...
    with col2:
        hydra_hit = st.number_input("通常時ヒドラ目からのczへの当選回数（高確でない）", 0, 1000, 3)
...
- ヒドラ目関連について、通常時の高確中にひいたヒドラ目は、出現当選ともにカウントしない前提で設計していますが、カウントしても別にいいです。（少し精度が悪くなりますが、高確滞在かどうかの判別は難しいので）
